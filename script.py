...
class HybridOsteoporosisDetector:
    ...
    def preprocess_xray(self, image: np.ndarray) -> Tuple[np.ndarray, np.ndarray]:
        """
        Pre-procesamiento específico para análisis óseo
    ...
    def enhance_trabecular_pattern(self, image: np.ndarray) -> np.ndarray:
        ...
        # Filtro Frangi para diferentes escalas de trabéculas
        for sigma in [0.5, 1.0, 1.5, 2.0]:
            try:
                enhanced += frangi(image, sigmas=[sigma], black_ridges=False)
            except:
                # Fallback si frangi falla
                kernel = cv2.getGaussianKernel(int(6*sigma), sigma)
                enhanced += cv2.filter2D(image.astype(np.float64), -1, kernel @ kernel.T)
    ...
    def generate_report(self, results: Dict, normal_image_path: str = "", 
        ...
        report = {
            ...
            "results": results,
            "summary": {
                "final_score": results.get('composite_score', {}).get('score', 0.0),
                "confidence": results.get('composite_score', {}).get('confidence', 0.0),
                "interpretation": results.get('composite_score', {}).get('interpretation', 'Unknown'),
                "recommendation": results.get('composite_score', {}).get('recommendation', 'Consult specialist')
            },
            "quality_metrics": {
                "normal_bone_coverage": results.get('preprocessing', {}).get('bone_coverage_normal', 0.0),
                "test_bone_coverage": results.get('preprocessing', {}).get('bone_coverage_test', 0.0),
                "regions_analyzed": results.get('regional_ks', {}).get('regions_analyzed', 0),
                "structural_features_extracted": len(results.get('structural', {}).get('changes', {}))
            }
        }
...
def main_example():
    ...
    try:
        # Cargar imágenes (reemplazar con rutas reales)
